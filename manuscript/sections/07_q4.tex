...
\papertable{tab_q4_ranges}{2026 年 9 月 11--17 日六个量的期望与 10\%--90\% 范围。左半为 7 日合计（点击量、展现量、浏览量、注册量）或\emph{单元--日层级}的点击加权平均（CPC、展位指数 $\pi$、上方位与首位占比），相对宽度 = 区间宽度 / 期望；CPC 行为“点击加权 CPC = 7 日总投放 / 7 日期望点击”，不是情景平均的 CPC 期望（后者见正文）。右半为 \valQfourRows{} 个关键词--日上的\emph{中位数}（等权，因而由长尾词主导，与点击加权的合计值不可直接比较）。每行取 4 位有效数字，行内左右两半一致；各行量纲相差 5 个数量级，故不统一小数位}{tab:q4-ranges}
