...
\paragraph{出价策略与预算}
消费在单元间同样集中：单元 \valBiggestUnitId{} 占全年消费 \valBiggestUnitSpendSharePct\%，各单元 CPC 从 \valMinUnitCpc{} 元到 \valMaxUnitCpc{} 元不等（表~\ref{tab:unit-kpis}）。第~\ref{sec:q1attr} 节估计的点击--消费弹性 $\gamma_u$ 全部小于 1（合并估计 \valPooledGamma，标准误 \valPooledGammaSe），即边际收益递减；而消费最大的单元 \valBiggestUnitId{} 的弹性最低（表~\ref{tab:attr}），其 CPC 也最高，说明预算在该单元上已进入边际成本快速上升的区域，把部分预算转移到弹性更高、CPC 更低的单元能以同样的钱换取更多点击与注册——这正是问题三、四的优化空间。
...
\end{equation}
合并模型（单元固定效应）给出 $\gamma=\valPooledGamma$（SE \valPooledGammaSe）；活跃天数不少于 60 且估计落在 $[0.3,0.95]$ 内的 \valUnitsWithOwnGamma{} 个单元采用自身估计（$\valMinUnitGamma$--$\valMaxUnitGamma$），其余（品牌型单元 \valTopCtrUnitId{} 的 $\hat\gamma>1$ 源于品牌流量的爆发式波动，另一单元活跃天数不足）采用合并估计。图~\ref{fig:resp} 表明双对数尺度上的线性关系在各单元都成立，$R^2$ 在 \valMinUnitGammaRsq--\valMaxUnitGammaRsq{} 之间（表~\ref{tab:attr}）。
