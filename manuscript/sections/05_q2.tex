...
\end{equation}
其中 $\tau_i$ 按题目定义是\emph{全体}访问的平均停留时间（跳出访问的时长已计入该平均），故 $C_i\tau_i$ 才是总停留时间，$C_i(1-b_i)\tau_i$ 应读作“非跳出访问次数 $\times$ 平均访问时长”，即有效注意力时长的一个下界代理；$r_{u(i)}$ 为所属单元的注册率（第~\ref{sec:q1attr} 节）；无访问记录（“/”）的词取 $b_i=1$、$\tau_i=0$。若改用总停留时间 $C_i\tau_i$ 作为 $z_{i3}$，五类标签与主口径的一致率为 \valEngagementVariantAgreementPct\%（表~\ref{tab:class-robust}），结论不受口径选择影响。对每个分量做极差标准化 $\tilde z_{ij}=(z_{ij}-\min_k z_{kj})/(\max_k z_{kj}-\min_k z_{kj})\in[0,1]$，权重由熵权法\cite{shannon1948}确定：
...
\mathcal B_i=\sum_{j=1}^{4}w_j\tilde z_{ij}\in[0,1].
\end{equation}
...
必须明确指出两个坐标轴\emph{并不独立}：四个效益分量都是规模量，与消费高度共单调——在 \valActiveKeywords{} 个有消费词上 $\operatorname{corr}(x^{c}_i,\mathcal B_i)=\valAxisPearson$（$R^2=\valAxisPearsonRsq$），Spearman 秩相关 \valAxisSpearman，其中点击量与效益指数的秩相关达 \valAxisSpearmanClicks。因此式~\eqref{eq:rule} 实质上是\textbf{沿同一条“消费--效益”主轴的分档，加上两个位于主轴两侧的小类}（黄金词与问题词），而不是两个独立维度的交叉分类。这不影响分类的可用性——题目要求的正是按“成本高低 $\times$ 效益高低”分组——但读者应据此理解两个偏离类的量级（第~\ref{sec:q2res} 节）。作为对照，若把效益轴换成\emph{效率}口径（每元归因注册的对数），其与成本的 Spearman 秩相关为 \valAxisSpearmanEfficiency（负相关），两种口径的标签一致率为 \valAxisEfficiencyAgreementPct\%，五类计数变为黄金 \valEffGoldCount、重点 \valEffKeyCount、潜力 \valEffPotentialCount、问题 \valEffProblemCount、无效 \valEffInvalidCount。效率口径下“高成本高效益”几乎为空（\valEffKeyCount{} 个），与题目“重点词＝高成本高效益”的设定不符，故主口径采用规模效益（MDR-0004）；两种口径并列供读者判断。
...
注意力时长 & $\log(1+C_i(1-b_i)\tau_i)$ & 非跳出访问数 $\times$ 平均访问时长 & \valWeightEngagement \\
...
\text{黄金词}, & x^{c}_i\le\tau_c,\ \mathcal B_i\ge\tau_b,\\
\text{重点词}, & x^{c}_i>\tau_c,\ \mathcal B_i\ge\tau_b,\\
\text{潜力词}, & x^{c}_i\le\tau_c,\ \mathcal B_i<\tau_b,\\
\text{问题词}, & x^{c}_i>\tau_c,\ \mathcal B_i<\tau_b,
\end{cases}
\end{equation}
...
\subsection{分类结果与稳健性}
\label{sec:q2res}
分类结果为：黄金词 \valGoldCount、重点词 \valKeyCount、潜力词 \valPotentialCount、问题词 \valProblemCount、无效词 \valInvalidCount（表~\ref{tab:class-summary}，图~\ref{fig:cost-benefit}；各单元的类别分布见附录表~\ref{tab:class-per-unit}）。\valKeyCount{} 个重点词承担了 \valKeySpendSharePct\% 的消费与 \valKeyClickSharePct\% 的点击，CPC \valKeyCpc{} 元，是账户的流量与注册主体；\valGoldCount{} 个黄金词的 CPC 仅 \valGoldCpc{} 元，以极低成本取得高于阈值的效益，但其经济量级很小——只占全年消费的 \valGoldSpendSharePctFine\% 与点击的 \valGoldClickSharePct\%，因此它们是\emph{值得小额扩量测试的候选}，扩量本身不会改变账户的消费结构；\valProblemCount{} 个问题词 CPC 高达 \valProblemCpc{} 元而效益低于阈值，应降低出价、收紧匹配或暂停，其消费占比同样只有 \valProblemSpendSharePct\%；\valPotentialCount{} 个潜力词消费与效益都低（CPC \valPotentialCpc{} 元），适合小预算测试以发现新的黄金词；\valInvalidCount{} 个无效词占词库四成，应清理或改用更宽的匹配方式。换言之，分类的即时财务影响集中在重点词的内部再分配上（问题三），两个偏离类的价值在于提供扩量与止损的候选清单。
